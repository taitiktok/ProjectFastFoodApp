...
public class edit_mon_an extends AppCompatActivity {
    private static final int REQUEST_CAMERA_PERMISSION = 100;
    private static final int REQUEST_IMAGE_CAPTURE = 101;
    private static final int REQUEST_IMAGE_PICK = 102;
    TextView IdeditMonAn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnAdd.setOnClickListener(v->{
               int idMonAnUpDate = Integer.valueOf(IdeditMonAn.getText().toString());
               String tenMonAnUpdate = edtTenMonAn.getText().toString();
               double giaTien = Double.parseDouble(edtGiaTien.getText().toString());
               String moTaChiTiet = edtMoTa.getText().toString();
               ...
               Boolean status = db.updateMonAnById(idMonAnUpDate,tenMonAnUpdate,giaTien,moTaChiTiet,imageBytes,maDanhMuc);
               if(status == true){
                   Toast.makeText(this, "Cập nhật thông tin món ăn thành công", Toast.LENGTH_SHORT).show();
                   finish();
               }
               else{
                   Toast.makeText(this, "Cập nhật thông tin món ăn không thành công ", Toast.LENGTH_SHORT).show();
               }
        });
    }
    ...
    private void mapping() {
        ...
        IdeditMonAn = findViewById(R.id.idMonEdit);
    }
    public void getDataNhanVienEdit(){
        Intent intent = getIntent();
        int id = intent.getIntExtra("IdMonAnEdit", -1);
        if (id == -1) {
            Log.d("Kh","Khong nhan duoc du lieu");
        }
        else{
            Log.d("Nhan dc du lieu",""+id);
            MySqliteDBDanhMuc db = new MySqliteDBDanhMuc(edit_mon_an.this);
        }
        MySqliteDBDanhMuc db = new MySqliteDBDanhMuc(edit_mon_an.this);
        MonAn monAn = db.getDataMonAnById(id);
        if (monAn != null) {
            IdeditMonAn.setText(String.valueOf(monAn.getMaMonAn()));
            edtTenMonAn.setText(monAn.getTenMonAn());
            edtGiaTien.setText(String.valueOf(monAn.getGiaTien()));
            edtMoTa.setText(monAn.getMoTaChiTiet());
            byte[] imageBytes = monAn.getImageMonAn();
            if (imageBytes != null && imageBytes.length > 0) {
                Bitmap bitmap = BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.length);
                imgMonAn.setImageBitmap(bitmap); // Đặt ảnh vào ImageView
            } else {
                // Nếu không có ảnh, có thể thiết lập ảnh mặc định
                imgMonAn.setImageResource(R.drawable.image_nv); // ví dụ ảnh mặc định
            }
        } else {
